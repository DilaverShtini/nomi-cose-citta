...
class LobbyScreen(BaseScreen):
    ...
    def _build_settings_controls(self):
        ...
        theme.separator(sf, color=theme.LINE_COLOR).pack(fill="x", padx=theme.PAD_MD, pady=(0, theme.PAD_SM))
    ...
    def _build_mode_selector(self):
        ...
        for value, name, desc in modes:
            ...
            tk.Radiobutton(row, text=name, variable=self._game_mode_var, value=value,
                           font=theme.FONT_BODY,
                           bg=theme.BG_PAGE, fg=theme.INK,
    ...
    def _build_num_categories_selector(self):
        ...
        wrap = tk.Frame(self._num_frame, bg=theme.BG_PAGE)
        wrap.pack(fill="x", padx=theme.PAD_MD)
        ...
        for n in range(MIN_EXTRA_CATEGORIES, MAX_EXTRA_CATEGORIES + 1):
            tk.Radiobutton(inner, text=str(n), variable=self._num_extra_var, value=n,
                           font=theme.FONT_BODY,
                           bg=theme.BG_PAGE, fg=theme.INK,
                           selectcolor=theme.YELLOW_HL,
                           activebackground=theme.BG_PAGE,
                           activeforeground=theme.BLUE_INK).pack(side="left", padx=6)
    ...
    def _build_categories_selector(self):
        ...
        self._categories_info = tk.Label(self._categories_frame, text="Extra categories:",
                                         font=(theme.HAND_FONT, 10, "bold"),
                                         bg=theme.BG_PAGE, fg=theme.BLUE_INK, anchor="w")
        self._categories_info.pack(fill="x", padx=theme.PAD_MD, pady=(theme.PAD_MD, 6))

        wrap = tk.Frame(self._categories_frame, bg=theme.BG_PAGE)
        wrap.pack(fill="x", padx=theme.PAD_MD)
        ...
        for i, cat in enumerate(AVAILABLE_EXTRA_CATEGORIES):
            ...
            tk.Checkbutton(grid, text=cat, variable=var,
                           font=theme.FONT_SMALL,
                           bg=theme.BG_PAGE, fg=theme.INK,
                           selectcolor=theme.YELLOW_HL,
                           activebackground=theme.BG_PAGE,
                           activeforeground=theme.GREEN_INK).grid(
    ...
    def _build_time_selector(self):
        ...
        self._time_scale = tk.Scale(row, from_=MIN_ROUND_TIME, to=MAX_ROUND_TIME,
    ...
    def _build_start_button(self):
        ...
        self._waiting_label = tk.Label(self._start_frame,
    ...
    def _handle_start_game(self):
        ...
        if mode in [GAME_MODE_CLASSIC_PLUS, GAME_MODE_FREE] and not selected:
            messagebox.showwarning("Selection required", "Select at least one category.")
            return
        config = {"mode": mode, "selected_categories": selected,
    ...
    def get_settings(self) -> dict:
        return {"mode": self._game_mode_var.get(),
